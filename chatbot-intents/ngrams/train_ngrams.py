...
#     if not os.path.isfile(OUTPUT_DIR + language + "_index_ngrams.pkl") or not os.path.isfile(OUTPUT_DIR + language + "_X_train_ngrams.pkl"):
    training_inds=None if len(training_inds)==1 else training_inds
    if recompute_index:
        ...
        pickle.dump(index, open(  OUTPUT_DIR  + language + "_index_ngrams_%i.pkl" % X_train.shape[0], "wb"))
        ...
        X_train=pickle.load(open( OUTPUT_DIR  + language + "_X_train_ngrams.pkl", "rb"))
        index=pickle.load(open(   OUTPUT_DIR  + language + "_index_ngrams_%i.pkl" % X_train.shape[0], "rb"))
        y_train=pickle.load(open(  OUTPUT_DIR  + language + "_train_multiclass_labels_merged.pkl", "rb"))
        X_test=pickle.load(open(  OUTPUT_DIR  + language + "_X_test_ngrams.pkl", "rb"))
        y_test=pickle.load(open(  OUTPUT_DIR  + language + "_test_multiclass_labels_merged.pkl", "rb"))
        word_counts=pickle.load(open (OUTPUT_DIR + language + "_wc.pkl", "rb"))
    ...
    pickle.dump(model, open(OUTPUT_DIR + language + "_ngrams_model_%i_%i.pkl" % (trial, X_train.shape[0]), "wb"))
    pickle.dump(history, open(OUTPUT_DIR + language + "_ngrams_history_%i_%i.pkl" % (trial, X_train.shape[0]), "wb"))
...
#             model, X_train, y_train, X_test, y_test, clean_labels=load_english_model(i==0, training_set, i)
        ...
        print_buff.append("IQR [%.2f]" % (100*scipy.stats.iqr(final_acc_scores)))
        print_buff.append("")
